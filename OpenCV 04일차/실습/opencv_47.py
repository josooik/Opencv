...
for i, contour in enumerate(contours):
   ...
   if area > 10:
       ...
       # 객체의 외각에 사각형 그리기
       # 방법 1 : boundingRect(회전 고려 않함)
       x, y, w, h = cv2.boundingRect(contour)
       cv2.rectangle(img_src1,  (x, y), (x+w, y+h), (255, 255, 0), 1)

       # 방법 2 : minAreaRect() 사용
       # 물체의 회전을 고려해서 경계 사각형 그림
       # boundingRect와 마찬가지로 2차원 포인터 집합을 입력으로 받는다. 이 함수는 입력받은 2차원 포인터 집합을 바탕으로 입력받은 포인터들을 모두 포함하면서 최소한의 영역을 차지하는 직사각형을 찾는다.
       # 이때 직사각형은 boundingRect 함수처럼 곧게 서있을 수도, 회전할 수도 있다. 좌상단의 x좌표, y좌표, 가로와 세로의 폭, 기울어진 각도 순으로 반환한다. 해당 반환값은 RotatedRect의 객체이다.
       rect = cv2.minAreaRect(contour)

       # boxPoints 함수는 회전된 직사각형의 4개의 꼭짓점을 찾는 함수로 회전된 직사각형을 그릴 때 유용하다.
       box = cv2.boxPoints(rect)
       box = np.int0(box)
       cv2.drawContours(img_src2, [box], 0, (255, 0, 255), 1)
       ...
       cont = cv2.hconcat([img_src, img_grays, mask, img_src1, img_src2])

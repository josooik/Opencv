...
class Ui(QtWidgets.QDialog):
    def __init__(self):
        ...
        uic.loadUi('PyQT/img_proc_qt2.ui', self)
    ...
    def btn_load_clicked(self):
        path = 'mov'
        filter = "All Images(*.mp4; *.mov; *.avi);;MP4 (*.mp4);;MOV(*.mov);;AVI(*.avi)"
        ...
        self.img_src = cv2.VideoCapture(filename)
        print(filename)

        while True:
            self.ret, self.img_frame = self.img_src.read()
            # self.img_frame = cv2.resize(self.img_frame, dsize=(554, 250), interpolation=cv2.INTER_AREA)
            self.display_output_image(self.img_frame, 0)

            if self.btn_run_flag == True:
